...
export async function loadDataAPI(
  ...
  // 如果需要转化参数可以在这里进行修改
  const res = await request('/api/v1/article_categories', {
    method: 'GET',
    params: {
      // ...params,
      page: params.current,
...
export function addDataAPI(data: any) {
  return request('/api/v1/article_categories', {
    method: 'POST',
    data,
  });
}
...
export function updateDataByIdAPI(id: string, data: any) {
  return request('/api/v1/article_categories/' + id, {
    method: 'PATCH',
    data,
  });
}
...
export function deleManyByIdsAPI(ids: string) {
  return request('/api/v1/article_categories/remove_many', {
    method: 'DELETE',
    params: {
      ids,
    },
